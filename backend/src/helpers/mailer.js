...
export const enviarCorreoResend = async (emailUsuario, ordenId, items, total) => {
  try {
    ...
    const logoPath = path.join(__dirname, '../assets/logo-mate-unico.png');
    const logoBuffer = fs.readFileSync(logoPath);

    const { data, error } = await resend.emails.send({
      from: "Mate Único <onboarding@resend.dev>",
      to: [emailUsuario],
      subject: `¡Compra Exitosa! Orden #${ordenId} 🧉`,
      html: htmlContent,
      attachments: [
    ...
    console.error("❌ Excepción enviando correo con Resend:", error);
    return false;
  }
};